...
(function () {
  ...
  function createDuck() {
    ...
    el.classList.add('floating-duck');
    ...
    return { el, x: startX, y, speed: fromLeft ? speed : -speed, fromLeft, bobAmp, bobFreq, phase: Math.random() * Math.PI * 2, baseY: y };
  }
  ...
  function animateDucks() {
    for (let i = ducks.length - 1; i >= 0; i--) {
      ...
      d.el.style.top = (d.baseY + Math.sin(d.phase) * d.bobAmp) + 'px';

      const gone = d.fromLeft ? d.x > window.innerWidth + 100 : d.x < -100;
      if (gone) {
  ...
  document.addEventListener('click', function (e) {
    for (let i = 0; i < 8; i++) {
      spawnSparkle(e.clientX + (Math.random() - 0.5) * 60, e.clientY + (Math.random() - 0.5) * 60);
    }
  });
  ...
  if (bandName) {
    setInterval(function () {
      if (Math.random() > 0.85) {
        ...
        const glitched = ORIGINAL_TEXT.substring(0, pos)
          + GLITCH_CHARS[Math.floor(Math.random() * GLITCH_CHARS.length)]
          + ORIGINAL_TEXT.substring(pos + 1);
        bandName.textContent = glitched;
        setTimeout(function () { bandName.textContent = ORIGINAL_TEXT; }, 80);
      }
    }, 200);
  }
